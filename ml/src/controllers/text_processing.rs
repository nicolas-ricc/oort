...
// You might need these struct definitions if they were originally in main.rs
...
// Define AppState if it was in main.rs
pub struct AppState {
...
// Shared function for common processing
pub async fn process_concepts_and_embeddings(
    ...
    if let Some(uid) = user_id {
        ...
        let user_concepts = state.db_client.get_user_concepts(uid).await?;
    ...
    if let Some(uid) = user_id {
        ...
        let user_id_owned = uid.to_string();
        ...
        tokio::spawn(async move {
            for (concept, embedding) in new_concepts_clone.iter().zip(new_embeddings_clone.iter()) {
                if let Err(e) = db_client.save_concept(&user_id_owned, concept, embedding).await {
...
// Simplified process_text handler
pub async fn process_text(
    data: web::Json<TextInput>,
    state: web::Data<AppState>,
) -> Result<impl Responder, ApiError> {
    process_concepts_and_embeddings(
        &data.text,
...
// Simplified upload_file handler
