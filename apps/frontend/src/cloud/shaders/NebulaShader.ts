...
export const nebulaFragmentShader = /* glsl */ `
  ...
  void main() {
    // Radial falloff from quad center
    float dist = length(vUv - 0.5);
    float radialFalloff = 1.0 - smoothstep(0.0, 0.5, dist);

    // 3D noise for cloud-like alpha
    vec3 noisePos = vec3(vUv * 3.0 + vSeed, uTime * 0.05);
    float noise = snoise(noisePos) * 0.5 + 0.5;
    noise *= snoise(noisePos * 2.0 + 10.0) * 0.5 + 0.5;

    float alpha = radialFalloff * noise * 0.12;
